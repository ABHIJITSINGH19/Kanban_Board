...
const Card = memo(function Card({
  ...
  useEffect(() => {
    if (status === "done" && isRunning && taskId) {
      dispatch(pauseTimerAPI(taskId));
    }
  }, [status, isRunning, taskId, dispatch]);
  ...
  if (variant === "list") {
    return (
      <div
        onClick={onClick}
        className={`group flex flex-col sm:flex-row items-start sm:items-center shadow-xl justify-between p-3 sm:p-4 bg-white cursor-pointer border border-black gap-3 sm:gap-0 ${className}`}
      >
        ...
        <div className="flex items-center gap-3 sm:gap-4 lg:gap-6 w-full sm:w-auto justify-end sm:justify-start">
          ...
          {status === "done" ? (
            <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3 font-mono text-xs sm:text-sm text-slate-600">
  ...
  return (
    <article
      onClick={onClick}
      className={`group relative bg-white border border-gray-100 rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-5 shadow-sm transition-transform duration-200 cursor-pointer w-full max-w-sm hover:scale-102 ${className}`}
    >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
        ...
        {status === "done" ? (
          <div className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full border border-slate-800 bg-slate-200">
