...
const DroppableColumn = ({
  ...
  return (
    <div
      ref={setNodeRef}
      className="flex flex-col gap-2 sm:gap-4 bg-black p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-800/50 min-h-[200px] shadow-2xl"
    >
...
const Tasks = () => {
  ...
  const columns = [
    { id: "todo", title: "To Do", color: "bg-white", status: "todo" },
    {
      id: "in-progress",
      title: "In Progress",
      color: "bg-yellow-500",
  ...
  const handleStatusUpdate = async (taskId, newStatus) => {
    ...
    const oldBackendStatus = task._backendTask.status;
    if (oldBackendStatus === backendStatus) return;

    if (
      oldBackendStatus === "completed" &&
      backendStatus === "in-progress" &&
      task._backendTask.timerState?.isRunning
    ) {
      await dispatch(pauseTimerAPI(taskId));
    }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
        <h2 className="text-xl sm:text-2xl font-bold text-slate-600">Tasks</h2>
        <div className="flex items-center gap-3">
          {user?.role === "manager" && (
            <>
              <button
                onClick={() => setCreateTaskOpen(true)}
                className="p-1.5 sm:p-2 flex items-center gap-1.5 sm:gap-2 rounded-lg hover:bg-black transition-colors text-white bg-gray-800 border border-slate-700 cursor-pointer"
              ...
              <div className="hidden sm:block w-0.5 h-8 bg-slate-200"></div>
            </>
          )}
        ...
        <div className="space-y-4 sm:space-y-6">
          {columns.map((section) => {
            ...
            return (
              <div
                key={section.id}
                className="bg-black border border-slate-800/50 rounded-xl sm:rounded-2xl"
              >
                <div className="w-full flex items-center justify-between p-3 sm:p-4 rounded-t-xl sm:rounded-t-2xl">
                  ...
                  <div
                    className="flex items-center cursor-pointer hover:bg-slate-800/50 rounded-md p-1 transition-colors"
